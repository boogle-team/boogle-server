...
export class GeminiTagExtractorService {
  ...
  private async requestExtraction(
    ...
    const prompt = [
      ...
      '문장에서 아래 7개 주제와 관련된 키워드를 최대 6개까지 한국어 태그로 추출해줘.',
      '- 음식/식사 (예: 야식, 매운음식, 회식, 치킨)',
      '- 수면 (예: 늦잠, 밤샘, 불면)',
      '- 스트레스/감정 (예: 스트레스, 예민함, 긴장)',
      '- 운동/활동 (예: 운동부족, 산책, 헬스)',
      '- 카페인 (예: 커피, 카페인)',
      '- 외출·여행 (예: 여행, 나들이, 외박)',
      '- 약·영양제 (예: 감기약, 유산균, 영양제)',
      '태그 작성 규칙:',
      '- 각 태그는 2자 이상 6자 이하의 짧은 명사형으로 작성해. 카테고리 이름을 그대로 쓰지 말고, 문장 내용에 맞는 자연스러운 단어로 표현해.',
      '- 사람 이름, 지명, 회사명 등 개인을 특정할 수 있는 정보는 절대 추출하지 마.',
      '- 각 태그마다 0과 1 사이의 신뢰도(confidence)를 함께 응답해.',
