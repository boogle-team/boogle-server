...
| 기능ID | 기능명 | category | linkTo |
| --- | --- | --- | --- |
| N101 | 위험 신호 알림 | `W` | `GUIDE_WARNING` |
| N102 | 기록 리마인더 | `R` | `HOME` |
| N103 | 리포트 도착 알림 | `P` | `REPORT` |
| N104 | PDF 저장 완료 알림 | `P` | `REPORT` |
| N105 | 연속 기록 독려 알림 | `R` | `HOME` |
...
| notifications[].category | string | Y | `W`(위험) / `R`(기록) / `P`(리포트) |
...
| alarm → (join) | alarm_id로 종류·제목·내용 조인 | category, title, content |
